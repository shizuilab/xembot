...
var v11 = new blynk.VirtualPin(11);//last_price
...
v1.on('read', function() {
  const fs = require('fs');
  const price = fs.readFileSync("/var/tmp/price.txt", {encoding: "utf-8"});
  v1.write(price);
});
...
v0.on('read', function() {
  ...
  var threshold_buy = parseFloat(fs.readFileSync("/home/pi/zaif/threshold-buy.txt"));
  var threshold_sell = parseFloat(fs.readFileSync("/home/pi/zaif/threshold-sell.txt"));
  v0.write(askbid);
  v9.write(timestamp);
  v10.write(history);
...
v13.on('write', function(param) {
  const fs = require('fs');
  fs.writeFileSync("/home/pi/zaif/threshold-buy.txt", param[0]/10000);
});

v14.on('write', function(param) {
  const fs = require('fs');
  fs.writeFileSync("/home/pi/zaif/threshold-sell.txt", param[0]/10000);
});
