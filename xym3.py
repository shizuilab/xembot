...
# 動的threshold-sell,buy = +- sigma > 廃止
# MACD > 2*sigma で逆張り
# XYMにスイッチ とりあえずxem.dbを引き継ぐ形で
...
def main():
    ...
    try:
        ...
        for stream in zaif_stream.execute(currency_pair=TARGET_CURRENCY_PAIR):
            ...
            bid_price = bids[0][0]
            ask_price = asks[0][0]
            mean_price = (ask_price + bid_price)/2 #統計にはaskとbidの中央値を使用する
            ...
            MACD = trade20 - trade200
            MACD2 = old_MACD2 + ALPHA3 * (MACD - old_MACD2) #MACDとして、MACDの移動平均を使う
            ...
            with open('/var/tmp/MACD.txt', 'w') as f:
                f.write(str(100000*(MACD - MACD2)/last_price)) #MACDとして、MACDの移動平均を使う
            ...
            if ask_pressure >= 50:
                mystr = '  ■ ジムの価格は' + str(round(mean_price,2)) + '円で、買いが強く' + str(ask_pressure) + '%です'

                print_red(mystr)
            else:
                mystr = '  ■ ジムの価格は' + str(round(mean_price,2)) + '円で、売りが強く' + str(100-ask_pressure) + '%です'
                print_green(mystr)

            if (MACD-MACD2)*10000 < 0:
                mystr = mystr + 'マックディーは、マイナス' + str(round((MACD - MACD2)*100000/last_price)) + 'です'
            else:
                mystr = mystr + 'マックディーは' + str(round((MACD - MACD2)*10000/last_price)) + 'です'
            ...
            mystr = '  ■ MACD - MACD2 = ' + str((MACD - MACD2)*10/last_price)
            if MACD - MACD2 > 0:
                print_red(mystr)
            elif MACD - MACD2 < 0:
                print_green(mystr)
            ...
            elif last_price < mean - (2 * sigma):
                with open('/var/tmp/ask-bid.txt', 'w') as f:
                    f.write('ask')
                print_green('◆◆◆ XYM 逆買いモード ◆◆◆', 1)

            elif last_price > mean + (2 * sigma):
                with open('/var/tmp/ask-bid.txt', 'w') as f:
                    f.write('bid')
                print_green('◆◆◆ XYM 逆売りモード ◆◆◆', 1)
